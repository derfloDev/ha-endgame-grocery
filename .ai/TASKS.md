...
| T-002 | Wire scan_interval into EndgameGroceryCoordinator (options → data → default); register async_reload_entry as options update listener; add/update tests in test_init.py and test_config_flow.py | done | Coordinator uses value from entry.options when present; falls back to entry.data then default 60 s; saving new interval in options triggers coordinator reload; all unittest tests pass | `python -m py_compile custom_components/endgame_grocery/__init__.py` OK; `python -m unittest tests.test_init` OK; `python -m unittest tests.test_config_flow` OK; `python -m unittest discover -s tests -p "test_*.py"` OK | none |
