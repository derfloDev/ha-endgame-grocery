...
| T-007 | Release Workflow — `.github/workflows/release.yml` | ready_for_implement | Workflow triggers on `v*.*.*` tag push; stamps version from tag into `manifest.json` workspace copy; zips `custom_components/endgame_grocery/` as `endgame_grocery.zip`; creates GitHub Release with ZIP asset and auto-generated release notes via `softprops/action-gh-release@v2` | Workflow YAML valid; ZIP contains `manifest.json` with correct version; GitHub Release created on tag push | implement |
| T-008 | README Overhaul & Logo — `README.md`, `custom_components/endgame_grocery/images/icon.png` | ready_for_implement | `README.md` completely rewritten: logo rendered, CI badge, HACS Open-in badge, Overview, Features, Prerequisites, HACS install steps, Manual install steps, Configuration, Usage, Troubleshooting, Release/versioning; AI Workflow section removed; `images/icon.png` is a valid PNG copy of `assets/endgame_grocery_logo.png`; existing test suite still passes | `python -m unittest discover` PASS; PNG validity check PASS; README renders correctly on GitHub | implement |
